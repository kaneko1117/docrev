...
ドキュメントをターミナルで開き、セルや行にコメントを付けると、AI エージェント(Claude など)が CLI 経由でそれを読み、対応し、返信します — 「コードレビューのドキュメント版」です。Excel ファイルはスプレッドシート風のグリッド(白いキャンバス・グリッド線・数式バー)で、Markdown ファイルは行番号付きで 1 行ずつ、ターミナル内に表示します。

![ビューアでセルにコメントすると、Claude が CLI 経由で読み取り、返信が自動でビューアに届く](demo/demo.gif)

> Excel(`.xlsx`)と Markdown(`.md`)の読み取り専用に対応。Word(`.docx`)対応は計画中です。
...
`add` は、指定したセルに既にスレッドがあればその続きに書き込みます(2 つ目のスレッドは作りません)。

Markdown ファイルでは、`--line` にビューアに表示される行番号(1 から数える)を渡します。`list --json` の各コメントには、セルの代わりに**その行の文字と前後 2 行ずつ**が同梱されます。ファイルを書き換えても、コメントは同じ行番号に残ったままで、文字を追いかけては動きません。
...
あとはビューアでセルや行にコメントを付けて、Claude に「budget.xlsx にコメントした」(または「notes.md にコメントした」)と伝えるだけ。返信はビューアが自動で拾って表示します。
